...
<body>
    ...
    <div class="container">
        ...
        <div class="add">
            <div class="row">
                <div class="col-6 col-sm-6 col-md-8 col-lg-10 p-0">
                    <h2>ราคาวัตถุดิบ</h2>
                </div>
                <div class="col-6 col-sm-6 col-md-4 col-lg-2 p-0">
                    <button type="button" class="btn btn-primary btn_add_raw" data-bs-toggle="modal"
                        data-bs-target="#addPriceModal"><i class="bi bi-plus-lg"></i>
                        เพิ่มราคาวัตถุดิบ
                    </button>
                </div>
            </div>
                ...
                if ($result) {
                    if ($result->num_rows > 0) { ?>
                        ...
                        while ($row = $result->fetch_assoc()) {
                            ...
                            <div class="modal fade" id="staticBackdrop<?php echo $idprice; ?>" data-bs-backdrop="static"
                                data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel<?php echo $idprice; ?>"
                                aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h1 class="modal-title fs-5" id="staticBackdropLabel<?php echo $idprice; ?>">แก้ไขราคา</h1>
                                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                        </div>
                                        <form action="priceupdate.php" method="post">
                                            <div class="modal-body">
                                                <?php
                                                $d = mysqli_query($conn, "select * from price where price_id='$sesprice'");
                                                $drow = mysqli_fetch_array($d);
                                                ?>
                                                <div class="mb-3">
                                                    <label class="form-label">ราคา</label>
                                                    <input type="text" class="form-control" name="price"
                                                        value="<?php echo $drow['price']; ?>" />
                                                </div>
                                            </div>
                                            <div class="modal-footer">
                                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">ยกเลิก</button>
                                                <input type="hidden" name="price_id" value="<?php echo $idprice; ?>">
                                                <button type="submit" class="btn btn-primary" name="change">อัปเดต</button>
                                            </div>
                                        </form>
                                    </div>
                                </div>
                            ...
                            <!-------------------------------------->
                            <div class="modal fade" id="delete<?php echo $idprice; ?>" data-bs-backdrop="static"
                                data-bs-keyboard="false" tabindex="-1" aria-labelledby="deleteLabel<?php echo $idprice; ?>"
                                aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h1 class="modal-title fs-5" id="deleteLabel<?php echo $idprice; ?>">ยืนยันการลบข้อมูล</h1>
                                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                        </div>
                                        <form action="price_delete.php" method="post">
                                            <div class="modal-body">
                                                <div class="m-3 text-center">
                                                    คุณต้องการลบข้อมูลราคา
                                                    <?php echo $row['raw_thainame']; ?> ใช่หรือไม่
                                                </div>
                                            </div>
                                            <div class="modal-footer">
                                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">ยกเลิก</button>
                                                <input type="hidden" name="price_id" value="<?php echo $idprice; ?>">
                                                <button type="submit" class="btn btn-danger" name="delete">ยืนยัน</button>
                                            </div>
                                        </form>
                                    </div>
                                </div>
                                ...
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        ...
                                        <form action="priceupdate.php" method="post">
                                            <div class="modal-body">
                                                <?php
                                                $d = mysqli_query($conn, "select * from price_ms where price_ms_id='$sesprice'");
                                                $drow = mysqli_fetch_array($d);
                                                ?>
                                                <div class="mb-3">
                                                    <label class="form-label">ราคา</label>
                                                    <input type="text" class="form-control" name="price_ms"
                                                        value="<?php echo $drow['price']; ?>" />
                                                </div>
                                            </div>
                                            <div class="modal-footer">
                                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">ยกเลิก</button>
                                                <input type="hidden" name="price_ms_id" value="<?php echo $idprice; ?>">
                                                <button type="submit" class="btn btn-primary" name="change">อัปเดต</button>
                                            </div>
                                        </form>
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <form action="price_delete.php" method="post">
                                            <div class="modal-body">
                                                <div class="m-3 text-center">
                                                    คุณต้องการลบข้อมูลราคา
                                                    <?php echo $rowMs['ms_thainame']; ?> ใช่หรือไม่
                                                </div>
                                            </div>
                                            <div class="modal-footer">
                                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">ยกเลิก</button>
