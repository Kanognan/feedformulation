...
<body>
    ...
    <div class="container">
        <div class="row" id="head">
            <div class="col-12">
                <?php
                $id = $_SESSION['acc_id'];
                $sql = "SELECT * FROM account INNER JOIN admin ON account.acc_id = admin.acc_id WHERE account.acc_id = '$id'";
                $rs = mysqli_query($conn, $sql);
                $result = mysqli_fetch_assoc($rs);

                $career_id = $result["career_id"];
                $sql_career = "SELECT * FROM career INNER JOIN admin ON career.career_id = admin.career_id WHERE career.career_id = '$career_id'";
                $rs_career = mysqli_query($conn, $sql_career);
                $result_career = mysqli_fetch_assoc($rs_career);

                $date = date('d F Y H:i:s', strtotime($result['admin_bdate']));
                $parts = explode(' ', $date);
                $day = $parts[0];
                $month = thai_month(date('m', strtotime($result['admin_bdate'])));
                $year = $parts[2] + 543;
                $time = $parts[3];
            ...
            <div class="container-personal">
                <div class="user-edit text-center">
                            ...
                            <div class="card card-body card-profile">
                                <div class="row infor">
                                    ...
                                    <div class="col-5" style="text-align: left;">
                                        <?php echo $result["admin_fname"];
                                        echo " ";
                                        echo $result["admin_lname"]; ?>
                                    </div>
                                </div>
                                <div class="row infor">
                                ...
                                <div class="row infor">
                                    ...
                                    <div class="col-2">:</div>
                                    <div class="col-5" style="text-align: left;">
                                        <?php echo $result["admin_phone"]; ?>
                                    </div>
                                </div>
                                <div class="col update text-center">
                                    <button type="button" class="btn btn-warning btn-edit-profile mt-1"
                                        data-bs-toggle="modal" data-bs-target="#editProfile">แก้ไขข้อมูล</button>
                    ...
                    <div>
                        <button class="btn btn-edit" type="button"
                            onclick="window.location.href='check_expert.php'">ตรวจสอบสิทธิ์ผู้เชี่ยวชาญ</button>
                    </div>
                    <div>
                        <button class="btn btn-edit" type="button"
                            onclick="window.location.href='expert_list.php'">จัดการสิทธิ์ผู้เชี่ยวชาญ</button>
                    </div>
                </div>
            </div>
        <?php } else {
            ...
            <div class="modal-content">
                ...
                <form action="profileEdit_model.php" method="POST" enctype="multipart/form-data">
                    <div class="modal-body">
                        ...
                        <div class="row">
                            <div class="mb-3 col">
                                <label for="admin_fname" class="col-form-label">ชื่อ :</label>
                                <input type="text" class="form-control" id="admin_fname" name="admin_fname"
                                    value="<?php echo $result["admin_fname"]; ?>">
                            </div>
                            <div class="mb-3 col">
                                <label for="admin_lname" class="col-form-label">นามสกุล :</label>
                                <input type="text" class="form-control" id="admin_lname" name="admin_lname"
                                    value="<?php echo $result["admin_lname"]; ?>">
                            </div>
                        </div>
                        <div class="row">
                            <div class="mb-3 col">
                                    ...
                                    <div class="input-group">
                                        <label class="radio-button">
                                            <div class="form-check form-check-inline gender">
                                                <input class="form-check-input" type="radio" name="admin_gender"
                                                    id="male" value="ชาย" <?php if ($result["admin_gender"] == "ชาย") {
                                                        echo "checked";
                                                    }
                                                    ?>>
                                        ...
                                        <label class="radio-button">
                                            <div class="form-check form-check-inline gender">
                                                <input class="form-check-input" type="radio" name="admin_gender"
                                                    id="female" value="หญิง" <?php if ($result["admin_gender"] == "หญิง") {
                                                        echo "checked";
                                                    }
                                                    ?>>
                                        ...
                                        <label class="radio-button">
                                            <div class="form-check form-check-inline gender">
                                                <input class="form-check-input" type="radio" name="admin_gender"
                                                    id="other" value="อื่นๆ" <?php if ($result["admin_gender"] == "อื่นๆ") {
                                                        echo "checked";
                                                    }
                                                    ?>>
                        ...
                        <div class="row">
                            <div class="mb-3 col">
                                <label for="admin_bdate" class="col-form-label">วัน/เดือน/ปีเกิด :</label>
                                <input type="date" class="form-control" id="admin_bdate" name="admin_bdate"
                                    value="<?php echo isset ($_POST['admin_bdate']) ? $_POST['admin_bdate'] : $result["admin_bdate"]; ?>">
                            </div>
                            <div class="mb-3 col">
                                <label for="admin_phone" class="col-form-label">เบอร์โทรศัพท์ :</label>
                                <input type="tel" class="form-control" id="admin_phone" name="admin_phone"
                                    value="<?php echo $result["admin_phone"]; ?>">
                            </div>
                        </div>
                        <div class="row image">
                            <div class="mb-3">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            var userBdateInput = document.getElementById('admin_bdate');
