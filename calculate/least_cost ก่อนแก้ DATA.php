...
<body>
                    ...
                    <table class="table table-light table-bordered table-responsive">
                        ...
                        <tbody class="text-center">
                                <tr>
                                    ...
                                    <td><?php echo $cow[0]['dem_intake']; ?></td>
                                    <td></td>
                                </tr>
                                <tr>
                                    <td>โปรตีนหยาบ (%)</td>
                                    <td><?php echo $cow[0]['dem_cp']; ?></td>
                                    <td></td>
                                </tr>
                        </tbody>
                        <tfoot>
    ...
    <script>
        ...
        // let hasDnutPremix = false;
        // constraints.Premix = { min: 0 };
            ...
            cowdemand.forEach(cow => {
                if (materials && materials.length > 0) {
                    materials.forEach(material => {
                        if (material.checkbox && material.checkbox.checkbox_nutrition && material.checkbox.checkbox_nutrition.length > 0) {
                            const dnut_tdn = material.checkbox.checkbox_nutrition.find(item => item.dnut_tdn);
                            const dnut_de = material.checkbox.checkbox_nutrition.find(item => item.dnut_de);
                            const dnut_me = material.checkbox.checkbox_nutrition.find(item => item.dnut_me);
                            const dnut_nel = material.checkbox.checkbox_nutrition.find(item => item.dnut_nel);
                            const dnut_cf = material.checkbox.checkbox_nutrition.find(item => item.dnut_cf);
                            const dnut_adf = material.checkbox.checkbox_nutrition.find(item => item.dnut_adf);
                            const dnut_ndf = material.checkbox.checkbox_nutrition.find(item => item.dnut_ndf);
                            const dnut_nfe = material.checkbox.checkbox_nutrition.find(item => item.dnut_nfe);
                            const dnut_cp = material.checkbox.checkbox_nutrition.find(item => item.dnut_cp);
                            const dnut_rup = material.checkbox.checkbox_nutrition.find(item => item.dnut_rup);
                            const dnut_ee = material.checkbox.checkbox_nutrition.find(item => item.dnut_ee);
                        ...
                        if (material.checkbox && material.checkbox.checkbox_material && material.checkbox.checkbox_material.length > 0) {
                            const dmat_lys = material.checkbox.checkbox_material.find(item => item.dmat_lys);
                            const dmat_met = material.checkbox.checkbox_material.find(item => item.dmat_met);
                            ...
                            if (dmat_met) {
                                constraints.MET.min = 100; //กำหนดค่าไว้ก่อน
                                hasDnutMet = true;
                                // console.log("สวัสดี2")
                            }
                        }
                        if (material.checkbox && material.checkbox.checkbox_minerals && material.checkbox.checkbox_minerals.length > 0) {
                            const dminer_per_ca = material.checkbox.checkbox_minerals.find(item => item.dminer_per_ca);
                            const dminer_per_p = material.checkbox.checkbox_minerals.find(item => item.dminer_per_p);
        ...
        [...materials, ...mineral_sources].forEach(item => {
            const isMaterial = item.hasOwnProperty('nutrition_detail_id') || item.hasOwnProperty('material_detail_id') || item.hasOwnProperty('minerals_detail_id');
            const nutrition = isMaterial ? item.checkbox.checkbox_nutrition || [] : [];
            const getNutritionValue = key => {
                const item = nutrition.find(item => item.hasOwnProperty(key));
                return item && !isNaN(parseFloat(item[key])) ? parseFloat(item[key]) : 0;
            };

            // const minerals = isMaterial ? [] : item.checkbox.checkbox_minerals || [];
            // const getMineralsValue = key => {
            //     const item = minerals.find(item => item.hasOwnProperty(key));
            //     return item && !isNaN(parseFloat(item[key])) ? parseFloat(item[key]) : 0;
            // };

            const detail = isMaterial ? item.checkbox.checkbox_material || [] : [];
            const getDetailValue = key => {
                const item = detail.find(item => item.hasOwnProperty(key));
                return item && !isNaN(parseFloat(item[key])) ? parseFloat(item[key]) : 0;
            };

            const minerals = item.checkbox.checkbox_minerals || [];
            ...
            variables[item.raw_thainame || item.ms_thainame] = {
                TDN: getNutritionValue('dnut_tdn'),
                DE: getNutritionValue('dnut_de'),
                ME: getNutritionValue('dnut_me'),
                NEL: getNutritionValue('dnut_nel'),
                CF: getNutritionValue('dnut_cf'),
                ADF: getNutritionValue('dnut_adf'),
                NDF: getNutritionValue('dnut_ndf'),
                NFE: getNutritionValue('dnut_nfe'),
                CP: getNutritionValue('dnut_cp'),
                RUP: getNutritionValue('dnut_rup'),
                LYS: getDetailValue('dmat_lys'),
                MET: getDetailValue('dmat_met'),
                CA: getMineralsValue('dminer_per_ca', minerals) || getMineralsValue('ds_ca', sourceMinerals),
                P: getMineralsValue('dminer_per_p', minerals) || getMineralsValue('ds_p', sourceMinerals),
                // Premix: 0,
                VitaminA: 20,
                VitaminD: 5,
                VitaminE: 2,
                EE: getNutritionValue('dnut_ee'),
                cost: [item.price],
