...
<head>
    ...
    <title>คำนวณโภชนะของโคแบบรายตัว</title>
    <style>
    * {
        margin: 0;
        padding: 0;
    ...
    <div class="flex">
        ...
        <div class="g-2">
            <div class="content">
                ...
                <form action="show_feed.php" method="post">
                    <?php
                    ...
                    <div class="feedcow">
                        <div class="row">
                            <h5>คำนวณโภชนะของโคแบบรายตัว</h5>
                            ...
                            <div class="row">
                                <?php
                                           $sql = "SELECT *, DATEDIFF(NOW(), cow_bday) AS age_in_days
                                           FROM cow 
                                           WHERE acc_id = $acc_id AND deleteAt IS NULL AND DATEDIFF(NOW(), cow_bday) >= 180";
                                           $result = $conn->query($sql);                                           
                                if ($result->num_rows > 0) {
                                ?>
                                <div class="col-lg-3 col-md-6 col-12 mb-3 was-validated">
                                    <label for="">เลือกโค</label><br>
                                    <div class="input-group">
                                        <select class="form-select" id="inputGroupSelect04"
                                            aria-label="Example select with button addon" name="cow_select" required>
                                ...
                                <div class="col-lg-2 col-md-6 col-12" id="gender">
                                </div><br><br>
                                <div class="col-lg-2 col-md-6 col-12 mb-3" id="cowAge">
                                </div><br><br>
                                <div class="col-lg-4 col-md-6 col-12 mb-3" id="cowGen">
                                </div>
                            </div>
                            <div class="row">
                            ...
                            <div class="row">
                                <div class="col-md-12 mb-1 was-validated" id="growth" name="growth"></div>
                            </div>
                            <div class="row ">
                        ...
                        <div class="d-flex justify-content-center btn-more">
                            ...
                            <div class="form-group">
                                <button type="submit" class="btn btn-add confirm" name="calproduct">คำนวณ</button>
                            </div>
                        </div>
                        <?php
                                            } else {
                                                ?>
                        <p style="text-align:center; color:gray; margin-top:2em"> คุณยังไม่มีข้อมูลโค ? <span><a
                                    style="text-decoration: underline; color:#6999C6" href="../cow/addcow.php">
                                    เพิ่มข้อมูลโค</a></span></p>
                        <?php
                                        }
                                        ?>
                    </div>
            ...
            document.getElementById("inputGroupSelect04").addEventListener("change", function() {
                ...
                if (selectedCow !== "") {
                    ...
                    sendDataToPHP(selectedCow);
                }

                function sendDataToPHP(selectedCow) {
                    const dataToSend = {
                        cowId: selectedCow,
                        gen: genContainer,
                        age: ageContainer,
                        gender: genderContainer,
                        calf_date: pregnancyDayContainer,
                ...
                function fetchCowData(selectedCow, genderContainer, ageContainer, genContainer,
                    ...
                    fetch(`fetch_cow_gender.php?selectedCow=${encodeURIComponent(selectedCow)}`)
                        ...
                        .then(data => {
                            ...
                            console.log(data);
                            const isPregnant = data.avg_gen.includes('โคพักรีด');

                            const cowGenders = Array.isArray(data.cow_gender) ? data
                                .cow_gender : [data.cow_gender];

                            const cowweight = Array.isArray(data.cow_weight) ? data
                                .cow_weight : [data.cow_weight];
                            ...
                            const calfdate = Array.isArray(data.calf_date) ? data
                                .calf_date : [data.calf_date];

                            const milkdate = Array.isArray(data.milk_date) ? data
                                .milk_date : [data.milk_date];
                            ...
                            cowweight.forEach(cow_weight => {
                                const weightInput = createInputField("inputweight",
                                    "น้ำหนักโค ณ ปัจจุบัน(กิโลกรัม)", cow_weight);
                                weightContainer.appendChild(weightInput);
                                weightInput.setAttribute("disabled", true);
                                weightContainer.appendChild(document.createElement(
                                    "br"));
                                const weightInputs = weightContainer
                                    .getElementsByTagName('input');
                            ...
                            breedstatus.forEach(cow_breed_status => {
                                const breedstatusInput = createInputField(
                                    "inputbreedstatus",
                            ...
                            milkstatus.forEach(cow_milk_status => {
                                const milkstatusInput = createInputField(
                                    "inputmilkstatus",
                            ...
                            milkdate.forEach(milk_date => {
                                const milkdayInput = createInputField("inputmilkdate",
                                    "วันให้นม(วัน)", milk_date);
                                milkdayContainer.appendChild(milkdayInput);
                                milkdayInput.setAttribute("disabled", true);
                                milkdayContainer.appendChild(document.createElement(
                                    "br"));

                                const milkdayInputs = milkdayContainer
                                    .getElementsByTagName('input');
                            ...
                            if (data.cow_milk_status == 'ไม่ให้นม' || data.cow_milk_status == 'พักรีด') {
                                // สร้าง input เพิ่ม
            ...
            function clearPreviousData() {
                ...
                document.getElementById("inputGroupSelect04").innerHTML = "";
